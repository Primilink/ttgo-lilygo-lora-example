...
void loop()
{
    ...
    if (Serial.available())
    {
        ...
        sending_packet["sender_id"] = "node_1"; // my id
        sending_packet["message"] = serial_message;

        // send packet
        LoRa.beginPacket();
        serializeJson(sending_packet, LoRa);
        LoRa.endPacket();
    ...
    if (packetSize)
    {
        ...
        while (LoRa.available())
        {
            ...
            DeserializationError error = deserializeJson(packet, LoRa);
            if (error)
            {
            ...
            String sender_id = packet["sender_id"];
            String incoming_message = packet["message"];

            // display packet on serial
